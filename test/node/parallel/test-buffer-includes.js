...
const allCodePoints = [];
for (let i = 0; i < 62000; i++) allCodePoints[i] = i;
...
for (let lengthIndex = 0; lengthIndex < lengths.length; lengthIndex++) {
  for (let i = 0; i < indices.length; i++) {
    ...
    // assert.ok(
    //     allCharsBufferUcs2.includes(patternBufferUcs2, 0, 'ucs2')); // to be fixed

    const patternStringUcs2 = patternBufferUcs2.toString('ucs2');
    // assert.ok(
    //     allCharsBufferUcs2.includes(patternStringUcs2, 0, 'ucs2')); // to be fixed
  }
}
...
// test truncation of Number arguments to uint8
{
  ...
  // assert.ok(buf.includes(0x697374657374)); // to be fixed

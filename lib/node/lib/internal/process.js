...
function lazyConstants() {
  if (!_lazyConstants) {
    _lazyConstants = native.require('constants').os.signals;
  }
...
function setupConfig(_source) {
  ...
  const processConfig = native.require('config');
...
function setupSignalHandlers() {
  ...
  process.on('newListener', function(type, listener) {
    if (isSignal(type) &&
      ...
      const Signal = native.require('signal_wrap').Signal;
